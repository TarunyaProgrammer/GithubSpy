...
export const ContributorList: React.FC<ContributorListProps> = ({ contributors, onSelectUser }) => {
  ...
  return (
    <div className="w-full my-6 bg-white dark:bg-obsidian-900 rounded-2xl border border-zinc-200/80 dark:border-zinc-800 p-5 shadow-xs">
      {/* Controls Bar */}
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 pb-4 border-b border-zinc-100 dark:border-zinc-800/80">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Role Filter Tabs */}
          <div className="flex items-center gap-0.5 p-1 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-xs" role="tablist" aria-label="Role Filter">
            <button
              role="tab"
          ...
          <select
            ...
            className="px-3 py-1.5 text-xs rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 focus:outline-none"
          >
            <option value="total">Sort: Most PRs</option>
            <option value="merged">Sort: Most Merged</option>
      ...
      {filteredContributors.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-4" role="list">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="relative">
                  <img
                    ...
                    alt=""
                    loading="lazy"
                    className="w-11 h-11 rounded-xl object-cover ring-1 ring-zinc-200 dark:ring-zinc-700"
                  />
                  {c.isMaintainer && (
                    <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-1">
                    <span className="font-semibold text-sm text-zinc-900 dark:text-white truncate font-mono group-hover:text-brand-500 transition-colors">
                      {c.username}
                    </span>
                    <a
                      ...
                      className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 p-0.5 rounded focus:outline-none focus-visible:ring-1 focus-visible:ring-brand-500"
                    >
                  ...
                  <div className="mt-2.5 grid grid-cols-3 gap-1 text-[11px] font-mono">
                    <div className="px-1.5 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-700/60 text-zinc-700 dark:text-zinc-300 text-center">
                      <span className="font-bold">{c.totalPRs}</span> total
                    </div>
                    <div className="px-1.5 py-0.5 rounded-md bg-violet-50 dark:bg-violet-950/40 text-violet-700 dark:text-violet-300 text-center">
                      <span className="font-bold">{c.mergedPRs}</span> mrg
                    </div>
                    <div className="px-1.5 py-0.5 rounded-md bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 text-center">
